...
import io.shcm.fabric.citresewn.pack.cit.effect.CITEffect;
...
public record CIT(CITCondition condition, Collection<CITEffect> effects) {
    public static final Codec<CIT> CODEC = RecordCodecBuilder.create(instance -> instance.group(
            ...
            Codecs.listOrSingle(CITEffect.CODEC)
                    .optionalFieldOf("apply", List.of()).forGetter(cit -> List.copyOf(cit.effects()))
